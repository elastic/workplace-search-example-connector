...
def cleanup(options)
  indexer = GitLabIndexer.new(
    ...
    workplace_search_api_access_token: options[:search_access_token],
...
def parse_options
  Slop.parse do |o|
    ...
    o.string '-s', '--search-access-token', 'The access token that can be used to call the Search API in Workplace Search', required: true
...
if $0 == __FILE__
  num_deleted = cleanup(parse_options)
  puts "Cleaned-up #{num_deleted} documents"
end
